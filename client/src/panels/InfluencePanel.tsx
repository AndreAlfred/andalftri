    ...
    <div className="space-y-6 text-white">
      <header className="space-y-4 border-b border-white/10 pb-5">
        <div className="space-y-2">
          ...
          <h2 className="text-3xl font-semibold text-white sm:text-4xl">{influence.title}</h2>
        </div>
        {influence.intro ? (
          <p className="max-w-3xl text-sm leading-7 text-white/78 sm:text-base">{influence.intro}</p>
        ) : null}
      </header>
      ...
      <section className="space-y-4">
        {groupedItems.map(([category, items]) => (
          ...
          >
            <p className="text-[0.72rem] uppercase tracking-[0.24em] text-white/45">{category}</p>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="text-base font-medium text-white sm:text-lg">{item.name}</h3>
                          {item.url ? (
                        ...
                        <p className="text-sm leading-7 text-white/72">{item.note}</p>
                      </div>
                      {item.url ? (
                        <span aria-hidden="true" className="pt-1 text-white/40">
                          ↗
                        </span>
                      ) : null}
                    </div>
